...
function tryFixPackages(config, packageDir, messages, cb) {
...
var resolve = module.exports = function resolveShims (file, messages, cb) {
  parentDir(path.dirname(file), 'package.json', function (err, packageDir) {
    ...
    try {
      ...
      shimField = pack['browserify-shim'];
    ...
    tryFixPackages(shimField, packageDir, messages, function (err) {
      if (err) return cb(err);
