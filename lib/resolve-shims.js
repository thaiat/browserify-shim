...
function customResolve(name, lookedUp, packageRoot, browserAlias, dependency, messages) {
  // Only custom resolve if we didn't use the package.json of the enclosing package of the file
  // This look up happens if we don't find a shim in the first package.json, but in the package enclosing the file's package
  // This fix is necessary since some non-commonJS packages (i.e. the ones downloaded with bower) contain package.json files
  if (!lookedUp) return name;
  ...
  try {
    ...
    return browserAlias 
      ...
      : dependency 
        ? resolve.sync(name, { basedir: packageRoot }) 

        // lets hope for the best that browserify will be able to resolve this, cause we can't
        : name;
  } catch (err) {
    // resolve.sync may fail, so what we need to do is just return the original name and hope for the best
    messages.push(format('WARNING, depends "%s" is found in package dependencies but could not be resolved.', name));
    return name;
  }
}

// TODO: stop the multi params madness, just create a hash
// TODO: trying to resolve here seems to not work properly although we seem to do it from the package root
function resolveDependsRelativeTo(dir, packageRoot, browser, depends, packDeps, lookedUp, messages) {
  var resolved;

  if (!depends) return undefined;

  return Object.keys(depends).reduce(function (acc, k) {
    var browserAlias = browser[k];
    if (browserAlias){
      acc[k] = customResolve(depends[k], lookedUp, packageRoot, browserAlias, null, messages);
      console.error('%s -> %s : %s -- %s [%s]', k, depends[k], acc[k], browserAlias, packageRoot);
      messages.push(format('Found depends "%s" exposed in browser field', k));
    } else if (!isPath(k)) {
      var dependency = packDeps[k];
      if (dependency) {
        acc[k] = customResolve(depends[k], lookedUp, packageRoot, null, dependency, messages);
...
function resolvePaths (packageDir, shimFileDir, browser, shims, packDeps, lookedUp, messages) {
  return Object.keys(shims)
    .reduce(function (acc, relPath) {
      ...
      var depends = resolveDependsRelativeTo(
          shimFileDir || packageDir
        , packageDir
        , browser
        , shim.depends
        , packDeps
...
function resolveFromShimFile(packageDir, pack, shimField, lookedUp, messages) {
...
function resolveInlineShims(packageDir, pack, shimField, lookedUp, messages) {
    ...
    parentDir(rootdir, 'package.json', function (err, packageDir) {
      ...
      try {
        ...
        var resolvedShims = typeof shimField === 'string'
          ? resolveFromShimFile(packageDir, pack, shimField, lookedUp, messages)
          : resolveInlineShims(packageDir, pack, shimField, lookedUp, messages);
